			...
			if($.browser.msie){
				...
				if(selectionParent.innerText.indexOf("/") >= 0){
					...
					if(sel.text.slice(-1)=="/"){
						...
						at['y'] = xyh.y;
						at['h'] = xyh.h;
						showTipGuide("请先输入数字编号（默认常用语）：<br/>1—常用语；<br/>2—ICD-10疾病；<br/>3—医嘱；");
			...
			if((at['mark']=="2" || at['mark']=="3") && $('#'+opts.tip).data(at['mark']+at['m'])!=null){
				...
				$.ajax({
					...
					success: function(data){
						if((at['mark']=="2" || at['mark']=="3"))$('#'+opts.tip).data(at['mark']+at['m'],data);
						generateHtml(data);
					},
					error:function(){
						alert("系统错误");
...
(function($) {
	...
	function check(o,event,a,b){
		...
		ajaxTime = window.setTimeout(function(){
			...
			if($(me).val()==""){
				if(opts.a=="1"){
					showTipGuide("请输入疾病拼音代码或名称后敲回车（Enter）检索");
	...
	function tipShow(){
		if(searchedm ==at['mark'] && searched == at['m']){tipBuild();}
			...
			if(at['mark']=="1"){
				//jsonurl = "emr/get_icd10.htm";
				jsonurl = "emr_new/get_icd10.htm";
			}else if(at['mark']=="2"){
				//jsonurl = "emr/get_icd9.htm";
				jsonurl = "emr_new/get_icd9.htm";
			}else{
				return false;
			}
